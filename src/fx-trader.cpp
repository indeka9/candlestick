...
std::vector<double> calculateEMA(const double* data, int period,int count) {
    std::vector<double> ema(count);
    double smoothingFactor = 2.0 / (period + 1.0);
    double sum = 0.0;

    // Initialize EMA with the first 'period' closing prices
    for (int i = 0; i < period; ++i) {
        sum += data[i];
        ema[i] = NAN; // Mark initial values as NaN
    }
...
void Demo_CustomPlottersAndTooltips()  {
    ...
    if (ImPlot::BeginPlot("Candlestick Chart",ImVec2(-1,0))) {
        ...
        // Calculate EMA (e.g., 20-day EMA)
        int emaPeriod = 64;  // Adjust the period as needed

  
        ImPlot::SetNextLineStyle(ImVec4(0, 0, 1, 1), 0.5f);         
        std::vector<double> emaData_highs = calculateEMA( highs, emaPeriod,218);
        ImPlot::PlotLine("EMA", dates, emaData_highs.data(), emaData_highs.size());


        ImPlot::SetNextLineStyle(ImVec4(0, 1, 0, 1), 0.5f);
        std::vector<double> emaData_lows = calculateEMA( lows, emaPeriod,218);
        ImPlot::PlotLine("EMA", dates, emaData_lows.data(), emaData_lows.size());

        ImPlot::SetNextLineStyle(ImVec4(1, 0, 0, 1), 0.5f);
        std::vector<double> emaData_closes = calculateEMA( closes, emaPeriod,218);
        ImPlot::PlotLine("EMA", dates, emaData_closes.data(), emaData_closes.size());
...
int main(int, char**) {
    // Create application window and ImGui context
    ...
    GLFWwindow* window = glfwCreateWindow(1280, 720, "ImPlot Candlestick Chart Demo", NULL, NULL);
